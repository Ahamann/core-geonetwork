    ...
    <xsl:template name="analyze-inspire-themes-and-topiccategory">
        ...
        <xsl:variable name="mappingAvailable">
            ...
            <xsl:for-each select="$root//gmd:topicCategory/gmd:MD_TopicCategoryCode">
                ...
                <xsl:for-each select="$itheme-topiccat-map/entry[topiccat = $tcId]/itheme">
                    ...
                    <xsl:variable name="themeLabel" select="$inspire-th//skos:Concept[@rdf:about = $themeId]/skos:prefLabel[@xml:lang = normalize-space($lang)]"/>
        ...
        <xsl:if test="normalize-space($mappingAvailable)!=''">
            <suggestion process="inspire-themes-and-topiccategory" category="keyword" target="keyword">
                <name>
                    <xsl:value-of select="geonet:i18n($itheme-topiccat-loc, 'a', $guiLang)"/></name>
                ...
                <xsl:for-each select="gmd:topicCategory/gmd:MD_TopicCategoryCode">
                    ...
                    <xsl:for-each select="$itheme-topiccat-map/entry[topiccat = $tcId]/itheme">
                        ...
                        <xsl:if test="count($mdKeywords//gmd:keyword[gco:CharacterString = $themeLabel]) != 1">
                            <elem><xsl:value-of select="$themeLabel"/></elem>
                        </xsl:if>
                    </xsl:for-each>
                </xsl:for-each>
            </xsl:variable>
                ...
                <xsl:when test="normalize-space($missing-inspirethemes) != ''">
                    <gmd:descriptiveKeywords>
                        <gmd:MD_Keywords>
                            ...
                            <xsl:for-each-group select="exslt:node-set($missing-inspirethemes)//elem" group-by=".">
                                <gmd:keyword>
                            ...
                            <xsl:choose>
                                <xsl:when test="$existingInspireThemes/gmd:MD_Keywords/gmd:thesaurusName">
                                    <xsl:copy-of select="$existingInspireThemes[1]/gmd:MD_Keywords/gmd:thesaurusName"/>
                                </xsl:when>
                                <xsl:otherwise>
                                    <gmd:thesaurusName>
                                        <gmd:CI_Citation>
                                            <gmd:title>
                                                <gco:CharacterString><xsl:value-of select="$inspire-th//skos:ConceptScheme/dc:title"/></gco:CharacterString>
                                            </gmd:title>
                                            <gmd:date>
                                                <gmd:CI_Date>
                                                    <gmd:date>
